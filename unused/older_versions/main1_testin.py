...
def calculate_tilt_angle_mod(ball_y, image_height, distance_mm):
    ...
    # Calculate the vertical distance to the ball (in mm) using trigonometry
    vertical_distance = distance_mm * math.tan(math.radians(vertical_angle)) + drop(distance_mm)
...
while True:
    ...
    if diff_t < 28:
        ...
        blobs = img.find_blobs([threshold_lab], pixels_threshold=40, area_threshold=40)
        if blobs:
            ...
            if round_bool == 1:
                ...
                if 5 <= len(avg_distance_list):
                    ...
                    s2.angle(-2*calculate_tilt_angle_mod(blobs[roundest_blob_index].cy(), image_height_pixels, avg_distance)+(avg_distance*0.01))

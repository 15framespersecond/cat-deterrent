...
s2 = pyb.Servo(1)   # create a servo object on position P7 TILT
...
threshold_lab = (54, 100, -78, -35, 28, 66) # 8:55AM
...
def calculate_tilt_angle4(distance_mm):
    tilt_angle = math.degrees((distance_mm * g) / (2 * water_velocity**2) - (2* turret_height / distance_mm))
    turret_drop = turret_height * math.tan(math.radians(tilt_angle))
    corrected_tilt_angle = math.degrees(math.atan2(turret_drop, distance_mm))
    return corrected_tilt_angle
...
def drop(distance_mm):
    tilt_angle = math.degrees((distance_mm * g) / (2 * water_velocity**2) - (2* turret_height / distance_mm))
    drop = turret_height * math.tan(math.radians(tilt_angle))
    #corrected_tilt_angle = math.degrees(math.atan2(turret_drop, distance_mm))
    return drop

def calculate_tilt_angle_mod(ball_y, image_height, distance_mm):
    # Calculate the vertical angle to the ball (in degrees)
    relative_y = (ball_y - (image_height / 2))  # Calculate the position relative to the image center
    vertical_angle = (relative_y / image_height) * vertical_fov

    # Calculate the vertical distance to the ball (in mm) using trigonometry
    vertical_distance = distance_mm * math.tan(math.radians(vertical_angle)) + drop(distance_mm)*0.5

    # Now, you can use trigonometry to calculate the tilt angle based on the vertical distance
    # and the distance to the ball (Pythagoras' theorem)
    tilt_angle = math.degrees(math.atan2(vertical_distance, distance_mm)*2.45)
        ...
        if blobs:
            ...
            if round_bool == 1:
                ...
                if 5 <= len(avg_distance_list):
                    ...
                    s1.angle(-calculate_pan_angle(blobs[roundest_blob_index].cx(), image_width_pixels, avg_distance)+5)
                    bound = -1.1*calculate_tilt_angle_mod(blobs[roundest_blob_index].cy(), image_height_pixels, avg_distance) + (avg_distance * 0.008)
                    if bound < 1:
                        blue_led.on()
                    else:
                        s2.angle(bound)
                        ...
                        red_led.off()
                        blue_led.off()
                    print("tilt angle: ", -calculate_tilt_angle_mod(blobs[roundest_blob_index].cy(), image_height_pixels, avg_distance))
                    #s2.angle(-2*calculate_tilt_angle3(avg_distance))
                    #print("tilt angle:",-calculate_tilt_angle3(avg_distance))
                    #s2.angle(-calculate_tilt_angle3(avg_distance) + 1)
                    avg_distance_list = []
                else:
